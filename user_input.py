...
class Input:
    ...
    def ReadEnvFromUser(self):
        while True:
            env = raw_input(
                "Please enter the Environment you want to have the template for:\n")
            if env.lower() not in ('development', 'experimental', 'production'):
                print(
                    "Not an appropriate choice. Valid choices are Development, Experimental or Production")
            else:
                break
    ...
    def ReadCIDRFromUser(self):
        while True:
            cidr = raw_input(
                "Please enter the CIDR you want to have for the VPC:\n")
